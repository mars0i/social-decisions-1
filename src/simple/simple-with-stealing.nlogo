...
to setup
  ...
  ask turtles [
    ...
    set steal-prob random-float max-steal-prob
    set amt-to-steal 5 ; change this
...
to go
  ...
  ask turtles [
    ...
    let theif-request amt-to-steal
    let stolen 0
    
    ask my-turtle-neighbors [
      if ((random-float 1) > [steal-prob] of myself) ; myself refers to the *asker*; self refers to the askee.
          and
         (theif-request <= wallet) [         ; if askee has at least as much as asker wants
           set wallet wallet - theif-request ; transfer the money from victim
           set stolen stolen + theif-request ; transfer the money to theif
      ]
    ]
    
    set wallet wallet + stolen ; add all stolen money into theif's wallet
    
    ; prevent irrational monetary amounts
    if wallet > max-wallet [set wallet max-wallet]
...
;; turtle reporter
to-report my-turtle-neighbors
  report turtles-on [neighbors4] of self
end
@#$#@#$#@
GRAPHICS-WINDOW
147
